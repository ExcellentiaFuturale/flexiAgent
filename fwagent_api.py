...
class FWAGENT_API(FwObject):
    ...
    def _get_lte_info(self, params):
        try:
            reply = fwglobals.g.modem_manager.get(params['dev_id']).get_lte_info()
    ...
    def _reset_lte(self, params):
        ...
        """
        try:
            fwglobals.g.modem_manager.get(params['dev_id']).reset_modem()
    ...
    def _modify_lte_pin(self, params):
        try:
            ...
            puk = params.get('puk')
            fwglobals.g.modem_manager.get(dev_id).handle_pin_modifications(current_pin, new_pin, enable, puk)
            reply = {'ok': 1, 'message': { 'err_msg': None, 'data': fwglobals.g.modem_manager.get(dev_id).get_pin_state()}}
        except Exception as e:
            reply = {'ok': 0, 'message': { 'err_msg': str(e), 'data': fwglobals.g.modem_manager.get(dev_id).get_pin_state()}}
        return reply
