...
def reset_modem_if_needed(err_str, dev_id):
    ...
    fwglobals.g.lte.get(dev_id).reset_modem()
    return True

def _run_qmicli_command(dev_id, flag, device=None):
    try:
        if not device:
            device = fwglobals.g.lte.get(dev_id).usb_device
...
def handle_unblock_sim(dev_id, puk, new_pin):
    ...
    updated_status, err = fwglobals.g.lte.get(dev_id).unblock_pin(puk, new_pin)
    if err:
...
def handle_change_pin_code(dev_id, current_pin, new_pin, is_currently_enabled):
    ...
    _, err = fwglobals.g.lte.get(dev_id).change_pin(current_pin, new_pin)
    if err:
        raise Exception(LTE_ERROR_MESSAGES.PIN_IS_WRONG)
...
def handle_verify_pin_code(dev_id, current_pin, is_currently_enabled, retries_left):
    updated_status, err = fwglobals.g.lte.get(dev_id).verify_pin(current_pin)
...
def handle_pin_modifications(dev_id, current_pin, new_pin, enable, puk):
    current_pin_state = fwglobals.g.lte.get(dev_id).get_pin_state()
...
def get_stats():
    ...
    for dev_id in dev_ids:
        ...
        try:
            info = fwglobals.g.lte.get(dev_id).get_lte_info()
            out[dev_id] = info
        except:
            pass
