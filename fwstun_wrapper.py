...
class FwStunWrap(FwObject):
    ...
    def find_addr(self, dev_id):
        ...
        : return :  local_ip associated with this dev id address -> str
                    ...
                    public_port of a local fwglobals.VXLAN_PORTS['port'] port or empty string -> int
    ...
    def _handle_stun_response(self, dev_id, p_ip, p_port, nat_type, st_index):
        ...
        if not cached_addr:
            return
        self.log.debug("found external %s:%s for %s:%s" %(p_ip, p_port, cached_addr['local_ip'], fwglobals.VXLAN_PORTS['port']))
        ...
        for dev_id in self.stun_cache:
            ...
            if time.time() >= cached_addr['send_time']:
                ...
                nat_type, nat_ext_ip, nat_ext_port, server_index = \
                    self._send_single_stun_request(local_ip, fwglobals.VXLAN_PORTS['port'], cached_addr['server_index'])
    ...
    def _stun_thread(self, *args):
        """STUN thread
        Its function is to send STUN requests for address:fwglobals.VXLAN_PORTS['port'] in a timely manner
    ...
    def _probe_symmetric_nat(self):
        ...
        for dev_id in list(self.sym_nat_cache):
            ...
            if time.time() >= cached_addr['probe_time']:
                ...
                src_port = fwglobals.VXLAN_PORTS['port']
