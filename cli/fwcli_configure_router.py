...
def argparse(configure_subparsers):
    ...
    create_interfaces_cli.add_argument('--do_not_expose_to_linux', dest='params.do_not_expose_to_linux', help="If it appears, VPPSB will not be created on Linux (Do it if you know what you are doing)", action='store_true')
...
def interfaces_create(type, addr, host_if_name, do_not_expose_to_linux=False):
    ...
    ret = daemon_rpc(
        ...
        type=type, addr=addr, host_if_name=host_if_name, do_not_expose_to_linux=do_not_expose_to_linux
    )
    return ret
...
def api_interface_create(type, addr, host_if_name, do_not_expose_to_linux, ospf=True, bgp=True, ):
    ...
    with FwCfgMultiOpsWithRevert() as handler:
        try:
            ...
            tun_vpp_if_name = handler.exec(
                ...
                params={ 'addr': addr, 'host_if_name': host_if_name, 'recreate_if_exists': True, 'no_vppsb_flag': do_not_expose_to_linux }
            )
            ...
            handler.exec(
                ...
                params={ 'add': True, 'vpp_if_name': tun_vpp_if_name, 'if_type': type },
